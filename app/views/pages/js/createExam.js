...
const questionsDiv = document.querySelector(".questions .scrollField");
...
function insertQuestionInQuestions(questions) {
    ...
    questions.forEach(question => {
        ...
        newQuestion.innerHTML = `
            <h3>Questão ${question.idQuestion}</h3>
            <p>${question.text}</p>
        `;
        ...
        newQuestion.addEventListener("click", () => {
            ...
            if (newQuestion.classList.contains("selected")) {
              if (selectedQuestions.length === 10) {
                sendButton.setAttribute("disabled", true);
              }
              newQuestion.classList.remove("selected");
              removeQuestionFromExam(newQuestion.id);
            } else {
              if (selectedQuestions.length === 10) {
                return;
              }
              ...
              insertQuestionInExam(newQuestion);
            }
...
async function getQuestions() {
  try {
    const response = await axios.get("http://localhost/lemonade/api/question", {
      headers: {
        ltoken: "b3050e0156cc3d05ddb7bbd9",
      },
    });
    console.log(response.data);
    insertQuestionInQuestions(response.data);
  } catch (error) {
    log(error);
  }
}
